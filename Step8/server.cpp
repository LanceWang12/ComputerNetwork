        ...
        while(1){
            ...
            for(i = 0; i < wnd; i++){
                //for( j = 0; j < 2; j++){
                    ...
                    /* loss positon  = 10204, 12288, 14336 */
                    if( dsize == 10240 || dsize == 12288 || dsize == 14336 ){
                        loss_flag = 0x80;   /* indicate loss */
                    }
                    else
                        loss_flag = 0;
                    /* send normal data, 0 or 0x80 */
                    snd_size = unit[id]->send_data(buffer, read_size, loss_flag);
                ...
                //}
                loss_flag = unit[id]->receive_data( );
                /* will trigger fast retransmit */
                if ( loss_flag & 0x80 ){
                    /* send all lose data in once */
                    unit[id] -> fast_retransmit_recovery(fptr);
                    break;
                }
                if(feof(fptr))
                    break;
